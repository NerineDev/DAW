...
async function buscar() {
  ...
  // Get filter state from UI, not just URL
  let modo = "normal";
  let orden = "desc";
  ...
  try {
    const res = await fetch(`negocio/api.php?search=${term}&modo=${encodeURIComponent(modo)}&orden=${encodeURIComponent(orden)}`);
...
`;
    ...
    $estado.textContent =
      modo === "filtro"
        ? `${data.length} resultado(s) (modo filtro activo).`
